...
import (
	...
	"github.com/kaolnwza/muniverse/order/lib/pkg"
)
...
func NewGinMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if claims["seer_uuid"] != "" {
			ctxSeerUUID, err := uuid.Parse(fmt.Sprintf("%s", claims["seer_uuid"]))
			if err != nil {
				log.Error(fmt.Errorf("middleware parse uuid err: %s", err.Error()))
				c.JSON(http.StatusBadRequest, err.Error())
				c.Abort()
			...
			ctxSeerKey := "access_seer_uuid"
			ctxSeer := context.WithValue(c.Request.Context(), ctxSeerKey, ctxSeerUUID)
			c.Request = c.Request.WithContext(ctxSeer)
		}
...
func (c *GinContext) AccessSeerUUID() uuid.UUID {
	userUUID, ok := c.Context.Request.Context().Value("access_seer_uuid").(uuid.UUID)
	if !ok {
		c.Status(http.StatusInternalServerError)
		c.Abort()
		return uuid.Nil
